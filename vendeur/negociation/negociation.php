  ...
  if(isset($_POST['envoie'])){
    ...
    if(isset($_FILES["itemphoto"]) AND !empty($_FILES["itemphoto"]["name"]))
   {
         ...
         if(in_array($extensionsUpload, $extensionsValides))
      {
         ...
         $reqitemnom = $bdd->prepare("SELECT * FROM items_negociation WHERE item_nom = ?");
         ...
         if($itemexiste==0){
	         if($resultat)
	         {
	            ...
	            $sql = $bdd->prepare("INSERT INTO items_negociation(item_nom,itemcat,itemphoto,item_desc,id_vendeur) VALUES ('$itemnom','$itemcat','$itemphoto','$item_desc','".$_SESSION['id_vendeur']."')");
	            $sql->execute(array($itemnom,$itemcat,$itemphoto,$item_desc));
...
<div class="wrapper">
    ...
    <div class="main_content">
        <div class="header">
          ...
          $liste = $bdd->query("SELECT item_nom,itemcat,itemphoto,item_desc FROM items_negociation WHERE id_vendeur = '".$_SESSION['id_vendeur']."' ");
          $tab = NULL;
           ...
           while ($produits = $liste->fetch())
            { 
                ...
                <th><img src="../../articlesimg/'.$produits['itemphoto'].'" style="width:50px;height:50px;"></th>
       ...
       ?>
          ...
          <div class="container ml-5 p-5">
            <button type="button" class="btn btn-block btn-secondary mb-1" data-toggle="modal" data-target="#modal-default" style="width: 100px;float:right;">Vendre</button>
        <table class="table"> 
