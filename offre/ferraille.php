...
<div class="wrapper">
    ...
    <div class="main_content">
        <div class="header">
          <?php
if(isset($_GET['error'])){
            $info = '<div class="alert alert-warning">Vous avez atteint la limite maximale d\'offre !</div>';
...
if(isset($_GET['error2'])){
            $info1 = '<div class="alert alert-warning">Vous avez déjà proposé une offre pour cet article !</div>';
     ...
     ?>
          ...
          <?php echo $info1; ?>
          <?php
          $liste = $bdd->query("SELECT * FROM items_negociation WHERE itemcat = 'Ferraille' ");
          $compteur=$liste->rowCount();
          $tab= NULL;
          if($compteur !=0){
	          while($produits = $liste->fetch())
	          {
	            $liste2 = $bdd->query("SELECT Nbessais,nomOffre FROM proposition_offre WHERE id_acheteur = '".$_SESSION['id_acheteur']."' AND nomOffre= '".$produits['item_nom']."' ");
	          $nbessais = $liste2->fetch(); 
	            $tab .= '<div class="col-sm-3">
	                        <div class="card mb-3">
	                          <form method="POST" action="faireOffreFer.php?id='.$produits['item_nom'].'">
	                          ...
	                          <div class="card-body">
	                            ...
	                            <div class="form-group">
	                              <center>Nombre d\'essais : '.$nbessais['Nbessais'].'</center>
